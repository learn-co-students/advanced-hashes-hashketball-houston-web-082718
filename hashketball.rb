...
def player_numbers(team_name)
	if game_hash[:home][:team_name].include?(team_name)
		game_hash[:home][:players].values.map {|player| player.fetch(:number)}
	elsif game_hash[:away][:team_name].include?(team_name)
		game_hash[:away][:players].values.map {|player| player.fetch(:number)}
	end
end
...
def big_shoe_rebounds
	...
	players.max_by{|name, stat| stat.fetch(:shoe)}[-1].fetch(:rebounds)
end